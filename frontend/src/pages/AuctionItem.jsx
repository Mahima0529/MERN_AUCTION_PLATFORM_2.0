...
>
 ...
 px-6 py-6 rounded-b-xl shadow-md min-h-[500px] max-w-[400px]  flex flex-col justify-start">
        ...
        {/* Place Bid Section */}
        <div className="bg-[#D6482B] py-2 px-2 max-w-[400px] rounded-2xl mt-2 flex flex-col md:flex-row md:items-center justify-between gap-4 text-white font-semibold">
          {Date.now() >= new Date(auctionDetail.startTime) &&
          Date.now() <= new Date(auctionDetail.endTime) ? (
            <>
              <div className="flex gap-3 items-center">
                ...
                <input
                  type="number"
                  className="w-28 text-black font-medium rounded-md focus:outline-none p-1"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                />
              </div>
              <button
                className="p-3 bg-black rounded-full hover:bg-[#222] transition-all duration-300"
                onClick={handleBid}
              >
            ...
            <p className="text-center w-full">Auction has not started yet!</p>
          ) : (
